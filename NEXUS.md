...
[*][b]DoctorKalle[/b] — author of [url=https://www.nexusmods.com/gothic1remake/mods/178]NoviceWaterMage[/url], which worked out how Gothic 1 Remake stores spell damage; the CDO-editing technique this mod uses to scale damage builds on theirs.
...
# Combined description (single block — paste straight into the Nexus description editor)
...
[list=1]
[*]Install [b]UE4SS[/b] for Gothic 1 Remake (if you don't have it yet).
[*]Extract the download so you end up with: [i]...\Gothic 1 Remake\G1R\Binaries\Win64\Mods\G1R_MageBalance\[/i] (containing [i]enabled.txt[/i] and [i]Scripts\[/i]).
[*]Start the game — the included [i]enabled.txt[/i] activates the mod.
[/list]
Want different numbers? Edit [i]Scripts\config.lua[/i] — one readable block per spell. No code knowledge needed.
